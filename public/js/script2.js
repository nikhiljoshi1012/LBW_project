document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("json-string-container").textContent = `Saved Data: ${JSON.stringify(savedData, null, 2)}`;

    document.getElementById("load-table-button").addEventListener("click", generateTableFromSavedData);

    var lastSavedData = JSON.stringify(savedData);
    ...
    function generateTableFromSavedData() {
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            for (let j = 0; j < columnCount; j++) {
                ...
                const cellKey = `${i}-${j}`;
                const fontFamily = i % 2 === 0 ? "ome_bhatkhande_hindi" : "Noto Sans Devanagari";
        ...
        document.getElementsByClassName("keyboard-container")[0].style.display = "flex";
        // Add event listeners for the new inputs
        const inputs = tbody.querySelectorAll("input");
        inputs.forEach((input, index) => {
            input.addEventListener("keydown", (event) => handleKeyDown(event, index, columnCount, inputs));

            input.addEventListener("input", (event) => captureInput(event, inputs, columnCount));
        });
    ...
    document.getElementById("save-option").addEventListener("click", function (event) {
        event.preventDefault();
    ...
    document.getElementById("copyProjectButton").addEventListener("click", function () {
    ...
    // Auto-save function to avoid unnecessary saves
    function autoSave() {
        const spinner = document.getElementById("autosave-spinner");
        const tableData = extractTableData();
        const tableDataString = JSON.stringify(tableData);

        if (tableDataString === lastSavedData) {
            console.log("No changes to save");
            return;
        }

        spinner.style.display = "block"; // Show the spinner
        lastSavedData = tableDataString;
        const projectDataInput = document.getElementById("project-data-input");
        projectDataInput.value = tableDataString;
