        ...
        <tbody>
                ...
                @foreach ($output as $item)
                    @if (is_array($item) || is_object($item))
                        <tr>
                            @foreach ($item as $subItem)
                                @php
                                    $decodedSubItem = json_encode($subItem, JSON_UNESCAPED_UNICODE);
                                @endphp
                    ...
                    @else
                        @if (!is_numeric($item))
                            <tr>
                                <td class="container" id="table-container" style="font-family: ome_bhatkhande_hindi">
                                    <code>{{ json_encode($item, JSON_UNESCAPED_UNICODE) }}</code>
                                </td>
                            </tr>
                        @endif
                    @endif
            ...
            @else
                @if (!is_numeric($output))
                    <tr>
                        <td class="container" id="table-container" style="font-family: ome_bhatkhande_hindi">
                            <code>{{ json_encode($output, JSON_UNESCAPED_UNICODE) }}</code>
                        </td>
                    </tr>
                @endif
            @endif
