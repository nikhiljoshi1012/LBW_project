              ...
              <div class="dropdown-item-text">
                  <div class="resizeInput-container">
                      ...
                      <div class="form-floating">
                        ...
                        <label for="resizeCol" style="font-size:small;">Columns</label>
                      </div>
                      <button class="btn btn-success" onclick="resizeTable()">Resize</button>                 
                  </div>
                </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container mt-3">
          ...
          <div class="row list-group-item list-group-item-action file-item">
            <div class="col">
              <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault" onchange="activeSelection({{ $proj->id }})">
            </div>
            <div class="col" onclick="location.href='{{ route('projects.show', $proj->id) }}'" style="cursor:pointer;">
              {{ $proj->name }}
            </div>
            <div class="col">{{ $proj->created_at }}</div>
            <div class="col">
              <a href="{{ route('projects.show', $proj->id) }}" class="btn btn-secondary btn-sm">View</a>
              <form action="{{ route('projects.destroy', $proj->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this project?')">X</button>
              </form>
            </div>
          </div>
          @endforeach
